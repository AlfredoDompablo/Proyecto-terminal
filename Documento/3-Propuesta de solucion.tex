...
En la figura \ref{Dig:diagrama general} se presenta un diagrama que muestra el funcionamiento general del sistema, destacando los diferentes módulos que lo componen: nodos sensores, nodo concentrador, almacenamiento en un servidor, así como la visualización a través de la página web.
...
El diseño del sistema  considera las recomendaciones establecidas con las normas \textit{NTC-ISO 5667-1:2023} \cite{ISO5667-1:2023} y \textit{NTC-ISO 5667-10:2020} \cite{ISO5667-10:2020}, que recomiendan el uso de instrumentos automáticos en línea o muestreos regulares para obtener datos representativos. En este marco, la red de sensores con transmisión remota asegura mayor frecuencia y calidad en la información recolectada.  

La frecuencia inicial de monitoreo propuesta es semanal, con la finalidad de obtener una mayor granularidad en los datos y detectar variaciones puntuales en el corto plazo. Este enfoque asegura un control más riguroso respecto a los intervalos establecidos en la \textit{NOM-001-SEMARNAT-2021}, cuyas exigencias mínimas para descargas (municipales y no municipales) oscilan entre frecuencias mensuales, trimestrales o semestrales dependiendo del tipo de carga contaminante \cite{nom001}. Los datos recopilados se enviarán a un servidor central para su análisis y publicación en una plataforma web accesible a autoridades y a la comunidad, permitiendo la visualización actualizada de la calidad del agua y las condiciones del cuerpo hídrico.
...
El nodo concentrador funge exclusivamente como el punto de enlace o pasarela (\textit{Gateway}) entre la red de sensores distribuida en el cuerpo de agua y la plataforma de almacenamiento en la nube. A diferencia de los nodos de monitoreo, este módulo no realiza la toma de muestras ambientales; su rol principal es centralizar el tráfico de datos, garantizando la integridad de la información durante su tránsito hacia el servidor.

\begin{itemize}
    \item \textbf{Hardware:} Unidad de procesamiento con interfaz de comunicación dual: radiofrecuencia de largo alcance para la red local y conexión IP (inalámbrica o cableada) para Internet. Opera con alimentación continua, prescindiendo de sistemas de recarga solar.
    
    \item \textbf{Control:} Opera bajo un esquema de escucha permanente, gestionando simultáneamente la recepción de telemetría de múltiples nodos y el mantenimiento del enlace de red con la nube.
    
    \item \textbf{Flujo de Información:} Actúa como gestor de tráfico: valida los datos entrantes, organiza la información y retransmite los datos estructurados al servidor central.
\end{itemize}

\subsection{Módulo 3. Servidor}
Es el centro de procesamiento y gestión, compuesto por:
\begin{itemize}
    \item \textbf{Base de Datos:} Almacena las series temporales de los parámetros medidos y las evidencias visuales.
    \item \textbf{Backend:} Realiza el procesamiento analítico de datos y el procesamiento de imágenes para la detección de residuos flotantes.
    \item \textbf{Frontend Web:} Sirve la interfaz de usuario para la visualización de estadísticas.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Residuos Flotantes:} Estimación del porcentaje de cobertura de residuos en la superficie del agua y galería de evidencia visual.
\end{itemize}
