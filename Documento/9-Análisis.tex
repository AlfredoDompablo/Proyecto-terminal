...
La Tabla~\ref{tab:comparativa_chips_tab:gnss_longtable} presenta un resumen de tecnologías GNSS de bajo consumo comúnmente utilizadas en sistemas IoT y WSN. Dichas soluciones se caracterizan por ofrecer un equilibrio entre sensibilidad, consumo y precisión métrica suficiente para aplicaciones ambientales, aunque presentan desafíos en zonas con follaje denso debido a pérdidas adicionales superiores a \SI{10}{dB} por atenuación vegetal.

\input{Documento/Tablas/Analisis/ComparativaGPS.tex}

\subsection{Justificación y estrategia de implementación}

Con base en la comparativa anterior, se selecciona el módulo \textbf{UC6580} como la solución GNSS para el sistema de monitoreo. Este módulo ofrece un equilibrio óptimo entre sensibilidad, consumo y tamaño, aspectos cruciales para nodos alimentados por baterías operando en entornos fluviales y áreas de difícil acceso.
...
\begin{enumerate}
    ...
    \item \textbf{Transmisión de datos eficiente:} Los datos del sensor (como pH, oxígeno disuelto, temperatura) se transmiten a través de la red LoRa con un bajo consumo energético, aprovechando la capacidad de bajo consumo del UC6580 y maximizando la vida útil de la batería.
