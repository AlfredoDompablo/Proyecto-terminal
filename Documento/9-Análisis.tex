...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SECCION: ANALISIS DEL SISTEMA Y REQUERIMIENTOS DEL PROTOTIPO %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Análisis del Sistema y Requerimientos del Prototipo}
\label{sec:analisis_del_sistema_y_requerimientos_del_prototipo}
...
\end{figure}
    ...
    \input{Documento/Tablas/Analisis/RangosParamCalidadAgua}
...
\input{Documento/Tablas/Analisis/Requerimientos2}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%    SECCION: SELECCION DE SENSORES     %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Selección de Sensores}
\label{sec:analisis_sensores}
Una vez establecidos los requerimientos funcionales y las variables a monitorear, el primer paso en el diseño de la arquitectura de hardware es la selección de los transductores físicos. Estos dispositivos serán los encargados de traducir los fenómenos químicos e hídricos en señales digitales cuantificables.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   SUBSECCION: ANALISIS SENSORES PH    %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Análisis Sensores pH}
\label{subsec:analisis_sensores_ph}
\input{Documento/Tablas/Analisis/Sensores/S.pH}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBSECCION: ANALISIS SENSORES OXIGENO DISUELTO %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Análisis Sensores Oxigeno Disuelto}
\label{subsec:analisis_sensores_oxigeno_disuelto}
\input{Documento/Tablas/Analisis/Sensores/S.OxigenDis}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBSECCION: ANALISIS SENSOR DE TURBIDEZ %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Análisis Sensor de Turbidez}
\label{subsec:analisis_sensor_de_turbidez}
\input{Documento/Tablas/Analisis/Sensores/S.turbidez}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBSECCION: ANALISIS SENSORES CONDUCTIVIDAD %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Análisis Sensores Conductividad}
\label{subsec:analisis_sensores_conductividad}
\input{Documento/Tablas/Analisis/Sensores/S.conductividad}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBSECCION: ANALISIS SENSORES TEMPERATURA %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Análisis Sensores Temperatura}
\label{subsec:analisis_sensores_temperatura}
\input{Documento/Tablas/Analisis/Sensores/S.temperatura}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     SECCION: ANALISIS DE CAMARAS      %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Análisis de Cámaras}
\label{sec:analisis_de_camaras}
De manera paralela a la adquisición de datos de calidad del agua, el sistema requiere recolectar evidencia visual en su entorno para automatizar la métrica de recolección de basura, labor que depende directamente de un sensor óptico.
...
\input{Documento/Tablas/Analisis/Cámaras2}

Con base en las restricciones de interfaz del microcontrolador seleccionado para la captura visual (ESP32-S3), el cual opera de manera óptima y nativa con buses periféricos paralelos (DVP) en lugar de arquitecturas MIPI CSI-2, se descartaron los módulos de cámara de Raspberry Pi. Dentro de las opciones DVP, se determinó utilizar la cámara \textbf{OV5640}. A pesar de presentar un consumo de corriente ligeramente superior frente a su predecesora (OV2640), esta cámara proporciona autoenfoque y una resolución nativa de 5 megapíxeles (2592$\times$1944). Estas métricas ópticas son indispensables para capturar detalles definidos sobre los cuerpos de agua, garantizando que el algoritmo YOLO disponga de información fidedigna para segmentar con exactitud el área cubierta por residuos.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SECCION: ANALISIS DE RECEPTORES GNSS  %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Análisis de Receptores GNSS}
\label{sec:analisis_gps}
Toda la telemetría recolectada por los sensores hídricos y visuales carece de valor analítico profundo si está desprovista de contexto espacial; por ende, resulta vital integrar un sistema capaz de geolocalizar estáticamente las posiciones desde donde se emiten dichas lecturas climáticas.

Para seleccionar la solución de posicionamiento más adecuada para el sistema de monitoreo, se evaluaron distintos módulos GNSS de bajo consumo comparables al integrado en la plataforma \textbf{Heltec Wireless Tracker V1.1}. El análisis considera tres criterios fundamentales:

\begin{enumerate}
    \item \textbf{Consumo energético}: Determina la autonomía del nodo durante el proceso de adquisición y seguimiento satelital, especialmente importante cuando el GPS se despierta periódicamente desde estados de bajo consumo.
    \item \textbf{Sensibilidad de recepción}: Capacidad del receptor para detectar señales GNSS de baja potencia, particularmente relevante en entornos con obstrucciones parciales como vegetación densa, humedad elevada y ausencia de cielo abierto total.
    \item \textbf{Tiempo a Primera Fijación (TTFF)}: Mide el tiempo necesario para obtener una posición válida. Influye directamente en el ciclo de operación del nodo y en el consumo energético total.
\end{enumerate}

La Tabla~\ref{tab:comparativa_chips_gnss} presenta un resumen de tecnologías GNSS de bajo consumo comúnmente utilizadas en sistemas IoT y WSN. Dichas soluciones se caracterizan por ofrecer un equilibrio entre sensibilidad, consumo y precisión métrica suficiente para aplicaciones ambientales, aunque presentan desafíos en zonas con follaje denso debido a pérdidas adicionales superiores a \SI{10}{dB} por atenuación vegetal.

\input{Documento/Tablas/Analisis/ComparativaGPS.tex}

Derivado del análisis comparativo previo, se opta por el módulo \textbf{UC6580} como la solución GNSS para el sistema de monitoreo. Este dispositivo presenta una relación idónea entre sensibilidad, consumo energético y dimensiones físicas, aspectos cruciales para nodos alimentados por baterías operando en entornos fluviales y áreas de difícil acceso.

Su sensibilidad de \SI{-165}{dBm} permite mantener una recepción estable incluso en escenarios con obstrucción parcial del cielo, como cuando hay árboles, humedad elevada y topografía irregular. Además, su soporte multiconstelación (GPS, GLONASS, Galileo) mejora significativamente la disponibilidad de satélites y reduce la probabilidad de pérdida de posicionamiento en zonas de vegetación densa, algo crucial en los entornos de monitoreo ambiental.

El consumo aproximado de \SI{25}{\milli\ampere} durante el seguimiento satelital, combinado con tiempos de adquisición de \SI{\sim 30}{\second} en \textit{cold start}, permite adoptar una estrategia eficiente basada en ciclos de activación breve y optimización energética.

La estrategia de operación se detalla a continuación:

\begin{enumerate}
    \item \textbf{Activación del GPS solo cuando sea necesario:} El módulo GNSS se activa únicamente para obtener las coordenadas de los sensores, y se apaga inmediatamente después de capturar la información. Esto reduce el consumo energético y prolonga la autonomía del sistema.
    
    \item \textbf{Esperar hasta obtener un \textit{fix} válido:} Antes de proceder con la transmisión de datos, el sistema espera a que el GPS obtenga una solución válida, minimizando el riesgo de transmitir coordenadas incorrectas.
    
    \item \textbf{Uso de un ciclo de baja potencia:} Una vez obtenido el fix, el GPS se apaga para entrar en el modo de baja potencia hasta que se necesite realizar una nueva adquisición de ubicación.
    
    \item \textbf{Posicionamiento y monitoreo remoto:} El UC6580 proporciona la precisión necesaria para el seguimiento geográfico a lo largo del río, facilitando la geolocalización precisa y el monitoreo en tiempo real de los nodos en la red.
\end{enumerate}

La elección del UC6580, al equilibrar sensibilidad, consumo y precisión, garantiza que el sistema de monitoreo se adapte eficazmente a las exigencias del entorno y optimice la eficiencia energética, asegurando así un rendimiento confiable y autonomía prolongada en zonas de difícil acceso.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SECCION: ANALISIS DE MICROCONTROLADORES %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Análisis de Microcontroladores} 
\label{sec:analisis_microcontroladores}   
Con el catálogo completo de instrumentos periféricos definido —sensores químicos, cámara de imagen y módulo GNSS— es inherente designar y evaluar a una central de procesamiento local capaz de orquestarlos a todos de manera equilibrada y eficiente.
...
\input{Documento/Tablas/Analisis/Microcontroladores2} 
...
Para el nodo sensor, se ha optado por una arquitectura de procesamiento distribuido utilizando dos unidades de desarrollo. Como controlador principal, encargado de la lectura de sensores físico-químicos (pH, turbidez, oxígeno disuelto) y la comunicación de largo alcance, se selecciona la placa Heltec Wireless Tracker V1.1. Esta elección se basa en su superioridad de integración, al unificar en un solo PCB el microcontrolador ESP32-S3, el transceptor LoRa SX1262 y el módulo GNSS UC6580, lo cual simplifica el diseño físico, elimina fallos por cableado externo y ofrece un consumo en reposo optimizado.

Para complementar este sistema, se selecciona la tarjeta de desarrollo Freenove ESP32-S3 dedicada exclusivamente a la captura y pre-procesamiento de imágenes. Al aislar las operaciones intensivas de control de la cámara y compresión de la imagen en esta segunda placa, se garantiza que el flujo crítico de comunicación LoRa y telemetría en la Heltec no se vea interrumpido por el manejo de archivos multimedia, resultando en un nodo más robusto y eficiente.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SECCION: ANALISIS DE LA TOPOLOGIA DE RED %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Análisis de la Topología de Red}
\label{sec:analisis_de_la_topologia_de_red}
Habiendo conceptualizado la estructura de hardware individual del nodo físico, nos enfrentamos al desafío de intercomunicar a estos agentes dentro de extensiones fluviales adversas. Para ello, es primordial esquematizar primeramente la estructura y flujo direccional del enrutamiento.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBSECCION: DESCRIPCION DE LA TOPOLOGIA SELECCIONADA %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBSECCION: JUSTIFICACION DE LA TOPOLOGIA LINEAL %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBSECCION: ESQUEMA DE COMUNICACION INTER-NODO %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBSECCION: IMPLICACIONES TECNICAS Y LIMITACIONES %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SECCION: ANALISIS DE TRANSCEPTORES INALAMBRICOS %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Análisis de Transceptores Inalámbricos}
\label{sec:analisis_tecnologias}
Con sustento en la topología lineal seleccionada para el seguimiento del cauce, el siguiente hito recae en elegir la interfaz electromagnética, un protocolo o tecnología física que pueda penetrar la vegetación garantizando las comunicaciones a largas distancias.
...
\input{Documento/Tablas/Analisis/Transceptores/ComparativaTransceptores}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SECCION: ANALISIS DE LA ALIMENTACION  %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Análisis de la Alimentación}
\label{sec:analisis_alimentacion}
Finalmente, como paso definitivo para cerrar y validar el esquema físico completo del hardware (microcontroladores, sensores y radios de alta potencia en la intemperie), se debe diseñar una matriz de ingeniería energética autosustentable que mantenga vivos estos elementos independientemente de las infraestructuras de la red eléctrica comercial.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBSECCION: REQUERIMIENTOS DE ALIMENTACION DE LOS COMPONENTES %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\input{Documento/Tablas/Analisis/ConsumosElectricos}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBSECCION: ANALISIS DE TIEMPOS DE TRANSMISION Y CARGA DE RED %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
El tiempo de transmisión se calcula con base en la tasa de datos efectiva ($R_b$) del transceptor \textbf{SX1262}. Conforme al análisis realizado en la \textbf{Sección \ref{sec:analisis_tecnologias}}, se utiliza la configuración de alta velocidad (SF7, BW \SI{500}{\kilo\hertz}, CR 4/5) para minimizar el tiempo de ocupación del canal, lo que proporciona una tasa de \SI[per-mode = symbol]{21.9}{\kilo\bit\per\second}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%    SUBSECCION: ESCENARIO OPERATIVO    %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\input{Documento/Tablas/Analisis/consumo-sem}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBSECCION: ANALISIS Y SELECCION DE COMPONENTES DE ALIMENTACION %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBSUBSECCION: ANALISIS Y SELECCION DE LA BATERIA %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Análisis y Selección de la Batería}
\label{subsubsec:analisis_y_seleccion_de_la_bateria}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBSUBSECCION: ANALISIS Y SELECCION DEL PANEL SOLAR %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Análisis y Selección del Panel Solar}
\label{subsubsec:analisis_y_seleccion_del_panel_solar}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBSUBSECCION: ANALISIS DE MODULOS DE CARGA %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Análisis de Módulos de Carga}
\label{subsubsec:analisis_de_modulos_de_carga}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SECCION: ANALISIS DE LA MEDICION DE RESIDUOS SOLIDOS FLOTANTES %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Análisis de la Medición de Residuos Sólidos Flotantes}
\label{sec:analisis_medicion_residuos}
Asegurado y resuelto el funcionamiento ininterrumpido del soporte en hardware en el río, el análisis muta e incursiona hacia el apartado analítico de métricas y \textit{software}. La meta imperante inicial recae formalmente en proyectar y matematizar la estimación concreta de qué representa una afectación por contaminación visual plástica.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SUBSECCION: CALCULO DE LA DENSIDAD DE COBERTURA %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SECCION: ANALISIS DE ARQUITECTURA DE COMPUTO PARA VISION ARTIFICIAL %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Análisis de Arquitectura de Cómputo para Visión Artificial}
\label{sec:analisis_edge_vs_cloud}
Una vez cimentado matemáticamente el indicador, se precisa ejecutar la segmentación de redes neuronales sobre los caudales. Esto despliega inevitablemente un dilema fundacional en aplicaciones de \textit{Internet of Things} modernas: ¿a qué nivel arquitectónico y bajo qué limitantes se procesarán algorítmicamente las pruebas?
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% SECCION: ANALISIS DE MODELOS PARA DETECCION Y SEGMENTACION DE OBJETOS %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Análisis de Modelos para Detección y Segmentación de Objetos}
\label{sec:analisis_vision}
Una vez determinado que el procesamiento de imágenes se realizará en la nube, el siguiente paso consiste en seleccionar el modelo de inteligencia artificial adecuado. Este algoritmo será el responsable de identificar y aislar visualmente los residuos flotantes dentro de cada imagen capturada.
...
Considerando la necesidad de segmentación, la madurez relativa dentro de las arquitecturas modernas y la robustez del framework, se selecciona \textbf{YOLOv8} como el modelo para este proyecto. Específicamente, se optará por una de sus variantes pre-entrenadas (ej. `yolov8s-seg.pt`) que ya incluye la capacidad de segmentación.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     SECCION: ANALISIS DE SERVIDOR     %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Análisis de Servidor}
\label{sec:analisis_de_servidor}
Habiendo seleccionado el modelo YOLO para la segmentación de imágenes, es necesario definir la infraestructura tecnológica que permitirá ejecutar el algoritmo de forma continua y confiable. Por lo tanto, se debe diseñar la arquitectura del servidor central que procesará estos datos.
...
\input{Documento/Tablas/Analisis/servidores.tex}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  SECCION: ANALISIS DE BASE DE DATOS   %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Análisis de Base de Datos}
\label{sec:analisis_db}
Finalmente, como complemento indispensable de la arquitectura del servidor, se requiere un sistema que pueda almacenar y organizar la información generada. Esta base de datos será responsable de salvaguardar el histórico de mediciones de los nodos y los resultados del análisis de visión artificial.
...
\input{Documento/Tablas/Analisis/ComparativaBD.tex}
