...
\subsection{Justificación y estrategia de implementación}
\label{subsec:justificacion_gps}

Con base en la comparativa anterior, se selecciona el módulo \textbf{Quectel L76K} como la solución GNSS para el sistema. Este módulo ofrece un equilibrio óptimo entre sensibilidad, consumo y tamaño, aspectos cruciales para nodos alimentados por baterías operando en entornos fluviales.

Su sensibilidad de \SI{-165}{dBm} permite mantener recepción estable aun en escenarios con obstrucción parcial del cielo debido a árboles, humedad y topografía irregular. Además, su soporte multiconstelación (GPS, GLONASS, Galileo) mejora la disponibilidad de satélites y reduce la probabilidad de pérdida de posicionamiento en zonas de vegetación densa.

El consumo aproximado de \SI{25}{\milli\ampere} durante el seguimiento satelital, combinado con tiempos de adquisición de \SI{\sim 30}{\second} en \textit{cold start}, permite adoptar una estrategia eficiente basada en ciclos de activación breve:

\begin{enumerate}
    \item Activar el módulo GNSS únicamente al momento de muestreo.
    \item Esperar hasta obtener un \textit{fix} válido (GGA Fix $\geq$ 1).
    \item Registrar coordenadas, número de satélites, HDOP y datos complementarios.
    \item Apagar el módulo para retornar al estado de bajo consumo del nodo.
\end{enumerate}

Aunque existen módulos con mayor precisión (u-blox M8) o menor consumo (Sony CXD5605), su integración implicaría antenas externas, mayor costo o rediseño de hardware, lo cual no se alinea con los objetivos de simplicidad y eficiencia energética del dispositivo.

Por lo tanto, el \textbf{L76K} representa la solución más adecuada para el contexto del despliegue fluvial, proporcionando precisión métrica, robustez ante atenuación vegetal y un consumo controlado acorde a los requisitos del sistema.
