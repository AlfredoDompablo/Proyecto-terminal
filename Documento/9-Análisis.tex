...
\midrule
\multirow{3}{*}{MCU ESP32-S3 (Heltec)} & Activo (IA) & 3.3 & 240.0 & \cite{heltec_wireless_tracker_2023} \\ 
...
Dentro de cada ciclo, el nodo alterna entre modos de bajo consumo (\textit{sleep}) y modos activos. Considerando el escenario más exigente (Nodo 4 retransmitiendo datos e imágenes de toda la red), el tiempo estimado en cada modo es el siguiente (ver Figura \ref{fig:ciclo_mod_op}):
...
\begin{itemize}
    ...
    \item \textbf{Tiempo en Modo Sleep}: 
    Considerando un ciclo base de \SI{1}{\hour} (para garantizar el balance energético con el panel solar):
    \[ \SI{60}{\minute} - \SI{7.4}{\minute} \approx \textbf{\SI{52.6}{\minute}} \]
    Esto representa aproximadamente el \SI{87.6}{\percent} del tiempo en reposo, lo cual sigue siendo sostenible con la generación solar proyectada.
\end{itemize}
