...
La arquitectura propuesta consiste en una red lineal multi-salto en la cual los nodos sensores se encuentran distribuidos a lo largo del cauce del río. Cada nodo es capaz de medir parámetros de calidad del agua, capturar imágenes del entorno y transmitir la información hacia un nodo concentrador.

Los nodos sensores incluyen los siguientes componentes principales:

\begin{itemize}
  \item Microcontrolador ESP32
  \item Módulo de comunicación LoRa
  \item Sensores de calidad del agua
  \item Cámara digital OV5640
  \item Sistema de alimentación basado en batería y panel solar
\end{itemize}

Los parámetros monitoreados por los sensores incluyen:
...
El nodo concentrador recibe la información transmitida por los nodos sensores y la envía a un servidor central mediante un enlace de mayor capacidad.

\section{Topología de la Red}

La red adopta una topología lineal debido a la naturaleza geográfica del escenario de despliegue, en el cual los nodos se ubican a lo largo del curso del río.

\begin{figure}[H]
  \centering
\begin{verbatim}
Nodo3 ---- Nodo2 ---- Nodo1 ---- Sink
\end{verbatim}
  \caption{Topología lineal de la red de sensores}
\end{figure}

En esta configuración, los nodos intermedios actúan como retransmisores, permitiendo la transmisión multi-salto de la información hacia el nodo concentrador.

\section{Diseño del Protocolo de Comunicación}

Para coordinar la transmisión de datos entre los nodos de la red se propone un protocolo de comunicación basado en acceso múltiple por división de tiempo (TDMA), sincronizado mediante mensajes de control denominados \textit{Beacon}.

\subsection{Mensajes Beacon}

El nodo concentrador transmite periódicamente un mensaje Beacon cuyo objetivo es sincronizar temporalmente a los nodos de la red e indicar el inicio de un nuevo ciclo de transmisión.

La estructura del mensaje Beacon se define en la Tabla \ref{tab:beacon}.

\begin{table}[H]
  \centering
  \caption{Estructura del mensaje Beacon}
  \label{tab:beacon}
  \begin{tabular}{|c|c|c|}
    \hline
    Campo & Tamaño (bytes) & Descripción \\
    \hline
    Tipo & 1 & Identificador del mensaje \\
    Timestamp & 4 & Tiempo de referencia del ciclo \\
    Periodo & 2 & Duración del ciclo de comunicación \\
    Slot\_time & 1 & Duración de cada slot TDMA \\
    CRC & 2 & Verificación de errores \\
    \hline
  \end{tabular}
\end{table}

\subsection{Acceso al Medio}

Después de recibir el mensaje Beacon, cada nodo transmite sus datos en un intervalo de tiempo previamente asignado. Los slots de transmisión se organizan desde el nodo más lejano hacia el nodo más cercano al concentrador.

Este esquema permite:

\begin{itemize}
  \item Evitar colisiones
  \item Reducir el consumo energético
  \item Garantizar transmisión determinista
\end{itemize}

\section{Transmisión de Imágenes}

Debido a las limitaciones del tamaño de carga útil de los paquetes LoRa, las imágenes capturadas por la cámara deben ser fragmentadas antes de su transmisión.

\subsection{Fragmentación}

La imagen capturada se comprime en formato JPEG y posteriormente se divide en fragmentos de tamaño fijo.
...
y el tamaño de cada fragmento es:

\[
  S_{frag}
\]

el número total de fragmentos se calcula como:

\[
  N_{frag} = \frac{S_{img}}{S_{frag}}
\]

Cada fragmento incluye un encabezado que contiene la información necesaria para su reensamblado.

\subsection{Formato de Fragmento}

La estructura de cada fragmento se muestra en la Tabla \ref{tab:fragmento}.

\begin{table}[H]
  \centering
  \caption{Formato de fragmento de imagen}
  \label{tab:fragmento}
  \begin{tabular}{|c|c|c|}
    \hline
    Campo & Tamaño (bytes) & Descripción \\
    \hline
    Node\_ID & 1 & Identificador del nodo \\
    Image\_ID & 1 & Identificador de imagen \\
    Fragment\_ID & 2 & Número de fragmento \\
    Total\_Fragments & 2 & Número total de fragmentos \\
    Payload & N & Datos de la imagen \\
    CRC & 2 & Verificación de errores \\
    \hline
  \end{tabular}
\end{table}

\section{Codificación de Corrección de Errores}

Con el objetivo de mejorar la confiabilidad de la transmisión en enlaces inalámbricos, se implementa un mecanismo de corrección de errores hacia adelante (FEC) basado en el algoritmo Reed-Solomon.

El algoritmo Reed-Solomon permite agregar símbolos redundantes a un bloque de datos con el fin de recuperar información perdida durante la transmisión.

\subsection{Modelo Reed-Solomon}

Un código Reed-Solomon se define como:
...
\begin{itemize}
  \item $k$ representa el número de símbolos de datos
  \item $n$ representa el número total de símbolos transmitidos
  \item $n-k$ corresponde a los símbolos de redundancia
\end{itemize}

La capacidad de corrección de errores está dada por:

\[
  t = \frac{n-k}{2}
\]

donde $t$ representa el número máximo de símbolos erróneos que pueden corregirse.

\subsection{Aplicación por Ventanas}

En el sistema propuesto, la codificación Reed-Solomon se aplica sobre ventanas de fragmentos de imagen.

Por ejemplo, una ventana puede estar compuesta por:

\begin{itemize}
  \item 10 fragmentos de datos
  \item 2 fragmentos de redundancia
\end{itemize}
...
Este esquema permite recuperar hasta dos fragmentos perdidos sin necesidad de retransmisión.

\section{Proceso de Transmisión}

El flujo general de transmisión se describe en la Figura \ref{fig:flujo}.
...
\end{verbatim}
  \caption{Flujo del proceso de transmisión}
  \label{fig:flujo}
\end{figure}

\section{Máquina de Estados del Nodo}

El comportamiento de los nodos sensores puede representarse mediante la máquina de estados mostrada en la Figura \ref{fig:estados}.
...
SLEEP
\end{verbatim}
  \caption{Máquina de estados del nodo sensor}
  \label{fig:estados}
\end{figure}

\section{Consideraciones Energéticas}

Dado que los nodos sensores operan con alimentación basada en batería y panel solar, el protocolo de comunicación fue diseñado para minimizar el tiempo de actividad de los nodos.

El uso de sincronización mediante Beacon y acceso TDMA permite que los nodos permanezcan en modo de bajo consumo durante la mayor parte del tiempo, activándose únicamente durante los intervalos de transmisión asignados.

\section{Resumen del Diseño}

El sistema propuesto integra una arquitectura de red lineal multi-salto con un protocolo de comunicación determinista basado en TDMA. La transmisión de imágenes se realiza mediante fragmentación y protección de errores utilizando códigos Reed-Solomon aplicados por ventanas de fragmentos.

Este enfoque permite mejorar la confiabilidad de la comunicación en enlaces LoRa de largo alcance, al tiempo que mantiene un consumo energético adecuado para nodos alimentados mediante sistemas autónomos.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
